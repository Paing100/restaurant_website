...
class NotificationControllerTest {
  ...
  void testSendNotification() throws Exception {
    ...
    mockMvc.perform(post("/api/notification/send").contentType("application/json").content(
        "{\"type\":\"type1\",\"orderId\":123,\"recipient\":\"recipient@example.com\",\"message\":\"This is a test message\", \"waiterId\": \"1\"}"))
        .andExpect(status().isOk());

    // Verify that the NotificationService's sendNotification method was called once with the
    // correct parameters
    verify(notificationService, times(1)).sendNotification("type1", 123, "recipient@example.com",
